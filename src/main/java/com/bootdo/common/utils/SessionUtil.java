...
public class SessionUtil {
    ...
    public UserDO getCurrentUser() {
        ...
        UserDO user = null;
        if (null != userBytes) {
            user = JSONSerializerUtil.unserialize(userBytes, UserDO.class);
        }
        return user;
    }


    public UserDO getCurrentUser(String token) {
        String redisKey = String.format(Constant.SESSION_REDIS_KEY, token);
        byte[] userBytes = redisLink.get(redisKey);
        UserDO user = null;
        if (null != userBytes) {
    ...
    public void setCurrentUser(HttpServletResponse response, UserDO suyUser) {
